...
/proc/admin_mob_info(mob/M, mob/user = usr)
	...
	to_chat(user, "(<a href='?src=\ref[usr];priv_msg=\ref[M]'>PM</a>) ")
	to_chat(user, "(<a href='?src=\ref[usr];adminplayeropts=\ref[M]'>PP</a>) ")
	to_chat(user, "(<a href='?_src_=vars;Vars=\ref[M]'>VV</a>) ")
	to_chat(user, "(<a href='?src=\ref[usr];subtlemessage=\ref[M]'>SM</a>) ")
	to_chat(user, "(<a href='?src=\ref[usr];adminplayerobservefollow=\ref[M]'>FLW</a>) ")
	to_chat(user, "(<a href='?src=\ref[usr];secretsadmin=check_antagonist'>CA</a>)")
