...
*/
		if(SHUTTLE_DOCKED)
			...
			if(time_left <= 0 && !shuttle_master.emergencyNoEscape)
				...
				send_to_info_discord("The Emergency Shuttle has left the station. Estimate [timeLeft(600)] minutes until the shuttle docks at Central Command.")
		if(SHUTTLE_ESCAPE)
			if(time_left <= 0)
				...
				send_to_info_discord("The game mode was [modename]!")
				timer = 0
				open_dock()
