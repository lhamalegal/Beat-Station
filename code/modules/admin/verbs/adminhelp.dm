...
/proc/send2irc_adminless_only(source, msg, requiredflags = R_BAN)
	...
	if(admin_number_present <= 0)
		if(!admin_number_afk && !admin_number_ignored)
			send_to_info_discord(source, "[msg] - No admins online")
		else
			send_to_info_discord(source, "[msg] - All admins AFK ([admin_number_afk]/[admin_number_total]) or skipped ([admin_number_ignored]/[admin_number_total])")
